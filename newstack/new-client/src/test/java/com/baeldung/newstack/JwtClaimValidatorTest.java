...
public class JwtClaimValidatorTest {
    ...
    private String obtainAccessToken(String username, String password) throws Exception {
        ...
        final Response response = RestAssured.given().auth().preemptive().basic(CLIENT_ID, CLIENT_SECRET)
                .with().params(params).when().post("http://localhost:8083/auth/realms/baeldung/protocol/openid-connect/token");
    ...
    @SpringBootApplication(scanBasePackageClasses = {AuthorizationServerApplication.class })
    public static class ClientAndAuthorizationServerApplication {
    }
